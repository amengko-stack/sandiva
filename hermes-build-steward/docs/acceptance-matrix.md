...
| AC-06 | Strict Build Task validator and canonical JSON schema, including complete immutable criterion evidence-authority policy | LOCAL PASS |
...
| AC-17 | Strict normalized result validation, unique evidence identities, exact criterion resolution, task-policy origin/kind enforcement and consistent dispositions | LOCAL PASS |
| AC-18 | Coordinator-owned immutable GitHub commit/check retrieval requires a task-authorized criterion before producing task-bound trusted-external CI evidence | LOCAL PASS; live evidence VM PENDING |
...
| AC-A6 | malformed/oversized results, dangling/duplicate/cross-criterion references, forged CI origins, unauthorized criterion self-vouching and job-supplied semantic evidence rejected before mutation | LOCAL PASS |
