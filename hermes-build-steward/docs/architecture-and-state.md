...
The immutable Canonical Build Task contains a complete `criterionEvidencePolicy`. Each criterion maps to one or more exact `{origin, kind}` pairs; independent origin and kind arrays are deliberately not used because their cross-product could grant unintended authority. Missing criteria, unknown criteria, malformed pairs, globally invalid origin/kind combinations, and duplicate pairs fail task validation. The policy is part of the canonical task fingerprint, so duplicate delivery cannot replace it. Result ingestion rejects every resolved evidence record whose exact origin/kind pair is not authorized for that criterion.

Repository JSON can supply only `isolated-job-observation` records in the deterministic-evidence channel. The coordinator adds `trustedOrigin: ISOLATED_VERIFICATION_JOB`; a repository-supplied origin or trusted evidence kind is rejected. Semantic evidence cannot come from the job result. `github-ci` evidence exists only when the coordinator's read-only GitHub client retrieves an immutable task-bound commit and its check runs, after which it is stamped `TRUSTED_EXTERNAL_SYSTEM`. The coordinator checks that every requested criterion permits that exact pair before making the external request. Lease, recovery, VM-runtime, audit, and reserved Partner Gate observations are constructed only on the trusted coordinator path and stamped `TRUSTED_COORDINATOR`. Kind labels do not create trust.
