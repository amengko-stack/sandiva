import { authorizeLitigation, litigationDenied, LitigationScopeError } from "@/lib/litigation-session";
import { NextRequest, NextResponse } from "next/server";
import { writeMatterFile } from "@/lib/litigation-sharepoint";

export const maxDuration = 30;

export async function POST(req: NextRequest) {
  try {
    const { sessionId, folderPath, filename, content, mimeType } = (await req.json()) as {
      sessionId: string;
      folderPath: string;
      filename: string;
      content: string;
      mimeType?: string;
    };

    if (!folderPath || !filename || content === undefined) {
      return NextResponse.json({ error: "folderPath, filename, dan content wajib diisi" }, { status: 400 });
    }

    const registration = await authorizeLitigation(sessionId, { root: folderPath, filename, folders: ["AI", "Drafts"] });

    const webUrl = await writeMatterFile(registration, filename, content, mimeType);
    return NextResponse.json({ ok: true, webUrl });
  } catch (e: unknown) {
    if (e instanceof LitigationScopeError) return litigationDenied();
    const message = e instanceof Error ? e.message : "Gagal menyimpan ke SharePoint";
    console.error("[save-matter-file] Error:", message);
    return NextResponse.json({ error: message }, { status: 500 });
  }
}
